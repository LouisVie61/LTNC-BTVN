...
const int TOO_BIG = 1, TOO_SMALL=-1, THE_SAME = 0, CHOOSED = 2;
void displayStartGame();
int letPlayerChoose(int markedNumbers[]);
int getComputerGuess(int lo, int hi);
void displayPlayerChoose(int number);
void displayComputerGuess(int number);
void updateAnswer(int &answer, int markedNumbers[], int number, int randomNumber);
void updateRange(int &lo, int &hi, int number, int answer);
...
SDL_Texture *bigMark, *smallMark, *sameMark, *mark;
...
int main(int argc, char* argv[])
{
    ...
    int randomNumber = letPlayerChoose(markedNumbers);
    displayPlayerChoose(randomNumber);

    int number, answer;
    int lo = 1, hi = 100;
    do {

        number = getComputerGuess(lo, hi);
        displayComputerGuess(number);
        updateAnswer(answer, markedNumbers, number, randomNumber);
        updateRange(lo, hi, number, answer);
...
void load_SDL_and_Images()
{
    ...
    mark = loadTexture("images/mark.bmp", renderer);
...
void markNumber(int i)
{
    ...
    } else if (markedNumbers[i]==CHOOSED){
        renderTexture(mark, renderer, x, y);
    }
}
...
void printComputerNumber(SDL_Texture *tex)
{
    renderTexture(tex, renderer, 575, 135);
    SDL_RenderPresent(renderer);
}
...
void printHumanMsg()
{
    renderTexture(humanMsg, renderer, 290, 100);
    SDL_RenderPresent(renderer);
}

void printHumanComparision(SDL_Texture *tex)
{
    renderTexture(tex, renderer, 290, 115);
    SDL_RenderPresent(renderer);
}
